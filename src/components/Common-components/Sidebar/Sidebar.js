...
const Sidebar = () => {
  ...
  const handleLogoutClick = () => {
    // Removed the setIsDropdownOpen line as it's not defined or needed
    setShowLogoutConfirmation(true);
  };

  const handleLogoutConfirm = () => {
    console.log("User confirmed logout");
    // Here you would typically call your logout API or clear auth state
  ...
  useEffect(() => {
    ...
    else if (path.includes('reports')) setActive('Reports');
  ...
  const menuItems = [
    { name: 'Dashboard', icon: <RxDashboard size={22} />, navigate: '/dashboard' },
    { name: 'Categories', icon: <TbCategory2 size={22} />, navigate: '/categories' },
    { name: 'Reports', icon: <MdOutlineReportGmailerrorred size={22} />, navigate: '/reports' },
    { name: 'Legal Policy', icon: <GoShieldLock size={22} />, navigate: '/policy' },
  ...
  return (
    <div className="h-screen w-[240px] bg-[#191919] text-white flex flex-col p-4">
      ...
      <button onClick={handleLogoutClick} className="mb-[160px] w-[192px] h-[48px] bg-[#474747] rounded-lg cursor-pointer flex items-center space-x-3 text-[#D9D9D9] hover:text-white">
