      ...
      <nav className="flex-1 mt-6">
        {menuItems.map((item) => (
          <Link to={item.navigate} key={item.name}>
            <div
              className={`flex items-center space-x-3 w-[192px] h-[48px] rounded-[4px] cursor-pointer transition-colors ${active === item.name ? "bg-[#06C4D9] text-white ease-out duration-300" : "text-[#D9D9D9]"
                }`}
